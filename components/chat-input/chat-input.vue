...
<view class="input-flex-column" @tap.stop>
    ...
    <view v-if="voiceObj.showCancelSendVoicePart" :style="'width: ' + voiceObj.voicePartWidth + 'px;height: ' + voiceObj.voicePartWidth + 'px;display: flex;position: fixed;left: ' + voiceObj.voicePartPositionToLeft + 'px;bottom: ' + voiceObj.voicePartPositionToBottom + 'px;justify-content:center;align-items: center;border-radius: 20rpx;'">
        ...
        <input v-else-if="inputStatus==='text'" class="chat-input-style" :style="'margin-left:' + (showVoicePart?0:16) + 'rpx;'" maxlength="500" confirm-type="send" :value="textMessage" @confirm="_chatInput$send$text$message" @focus="_chatInput$bind$focus$event" @blur="_chatInput$bind$blur$event" @input="_chatInput$getValue$event"></input>
